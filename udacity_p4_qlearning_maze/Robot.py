...
class Robot(object):
    ...
    def update_parameter(self):
        """
        Some of the paramters of the q learning robot can be a
ltered,
    ...
    def choose_action(self):
        ...
        if self.learning:
            if is_random_exploration():
                ...
                actions = ['u','r','d','l']
                self.action = actions[random.randint(0,3)]
                return self.action
            else:
                # TODO 7. Return action with highest q value
                action_dict = self.Qtable[self.state]
                self.action = max(action_dict, key = action_dict.get)
                return self.action
        elif self.testing:
